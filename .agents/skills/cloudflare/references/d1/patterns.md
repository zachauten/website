...
async function searchUsers(
  ...
  const whereClause = conditions.length > 0
    ...
    : "";
  return await env.DB.prepare(`SELECT * FROM users ${whereClause}`).bind(
    ...params,
  ).all();
}
```
...
async function getCachedUser(
  ...
  const user = await env.DB.prepare("SELECT * FROM users WHERE id = ?").bind(
    userId,
  ).first();
  if (user) {
...
const users = await env.DB.prepare("SELECT * FROM users WHERE email = ?").bind(
  email,
).all();
...
for (const post of posts) {
  const author = await env.DB.prepare("SELECT * FROM users WHERE id = ?").bind(
    post.user_id,
  ).first(); // Bad: multiple round trips
}
...
export default {
  async fetch(
    ...
    const data = await env[`TENANT_${tenantId}`].prepare(
      "SELECT * FROM records",
    ).all();
...
async function runMigration(env: Env) {
  ...
  try {
    ...
    await session.prepare("CREATE INDEX idx_posts_user ON posts(user_id)")
      .run();
  ...
  try {
    ...
    while (true) {
      const rows = await session.prepare(
        "SELECT id, data FROM legacy LIMIT ? OFFSET ?",
      ).bind(BATCH_SIZE, offset).all();
      if (rows.results.length === 0) break;
      const updates = rows.results.map(row =>
        session.prepare("UPDATE legacy SET new_data = ? WHERE id = ?").bind(
          transform(row.data),
          row.id,
        )
      );
