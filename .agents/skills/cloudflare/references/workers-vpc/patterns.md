...
```typescript
const socket = connect({ hostname: "echo.example.com", port: 7 }, {
  secureTransport: "on",
});
try {
  await socket.opened;
...
const socket = connect({ hostname: "stream.internal", port: 9000 }, {
  secureTransport: "on",
});
const writer = socket.writable.getWriter();
...
async function connectWithFallback(
  ...
  try {
    const socket = connect({ hostname: primary, port }, {
      secureTransport: "on",
    });
    await socket.opened;
    return socket;
