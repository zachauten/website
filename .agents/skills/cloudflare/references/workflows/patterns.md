...
export class DataPipelineWorkflow extends WorkflowEntrypoint<Env, Params> {
  async run(event, step) {
    const rawData = await step.do("extract", {
    ...
    const transformed = await step.do(
      "transform",
      async () =>
...
await step.do(
  "other work",
  async () => console.log(`Child started: ${child.id}`),
);
```
