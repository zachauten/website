...
// D1
const result = await env.DB.prepare("SELECT * FROM users WHERE id = ?").bind(1)
  .first();

// D1 Sessions (2024+) - read-after-write consistency
const session = env.DB.withSession();
await session.prepare("INSERT INTO users (name) VALUES (?)").bind("Alice")
  .run();
const user = await session.prepare("SELECT * FROM users WHERE name = ?").bind(
  "Alice",
