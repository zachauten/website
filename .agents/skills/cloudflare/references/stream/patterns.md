...
export function VideoPlayer(
  { videoId, token }: { videoId: string; token?: string },
) {
  return (
    <Stream
      controls
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (
      !signature || !await verifyWebhook(signature, body, env.WEBHOOK_SECRET)
    ) {
...
async function selfSignToken(
  ...
  const header = btoa(JSON.stringify({ alg: "RS256", kid: keyId })).replace(
    /=/g,
    "",
  ...
  const b64Sig = btoa(String.fromCharCode(...new Uint8Array(sig))).replace(
    /=/g,
    "",
  ).replace(/\+/g, "-").replace(/\//g, "_");
