...
function updateStage(topSpeakers: string[]) {
  const toAdd = topSpeakers.filter(id => !activeSubscriptions.has(id)).slice(
    0,
    6,
  );
  const toRemove = [...activeSubscriptions].filter(id =>
    !topSpeakers.includes(id)
  );
